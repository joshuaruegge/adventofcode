...
public class Day15 implements IDay{
	...
	public static ArrayList<Coord> unexploreds () {
		...
		for(Coord c : airPositions) {
			for(int x = -1; x < 2; x++) {
				for(int y = -1; y < 2; y++) {
					if(x == 0 ^ y == 0) {
						Coord neighbor = new Coord(c.x + x, c.y + y);
	...
	public Coord adjacentAir(Coord c) {
		for(int x = -1; x < 2; x++) {
			for(int y = -1; y < 2; y++) {
				if(x == 0 ^ y==0) {
					Coord neighbor = new Coord(c.x + x, c.y + y);
					if(airPositions.contains(neighbor))
						return neighbor;
				}
			}
		}
	...
	//return a list representing path between start and end, using pseudo-Dijkstra
	public static ArrayList<Coord> pathBetween(Coord start,Coord end) {
		ArrayList<PathNode> opens = new ArrayList<PathNode>();
		HashSet<PathNode> closeds = new HashSet<PathNode>();
		//we'll try this with no heuristic for now
		opens.add(new PathNode(start,0,0));
		while(opens.size() > 0) {
			PathNode current = opens.remove(0);
			Coord curPos = current.pos;
			if(curPos.equals(end)) {
				//build path from nodes in PathNode object
				ArrayList<Coord> path = new ArrayList<Coord>();
				for(PathNode n : current.path)
					path.add(n.pos);
				return path;
			}
			...
			for(int xOff = -1; xOff < 2; xOff++) {
				for(int yOff = -1; yOff < 2; yOff++) {
					if(xOff == 0 ^ yOff == 0) {
						...
						if(airPositions.contains(neighbor)) {
							//create new node (which copies over previous node's path) and add itself to the path
							PathNode newNode = new PathNode(current,neighbor, current.gcost + 1, 0);
							newNode.path.add(newNode);
							if(!opens.contains(newNode) && !closeds.contains(newNode)) {
								opens.add(newNode);
							}
							//no need to worry about faster-than-discovered conditions probably. hopefully.
						}
					}
				}
			}
		...
		while(oxygen.size() < airPositions.size()) {
			...
			for(Coord c : oxygen) {
				for(int xOff = -1; xOff < 2; xOff++) {
					for(int yOff = -1; yOff < 2; yOff++) {
						if(xOff == 0 ^ yOff == 0) {
							Coord next = new Coord(c.x + xOff, c.y + yOff);
