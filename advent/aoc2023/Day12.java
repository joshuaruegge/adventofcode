...
public class Day12 implements IDay {
    ...
    public String part1() {
        ...
        for(String line: input.split("\n")) {
            ...
            long result = memoizedRecurse(new HashMap<>(),parts[0]+".",count, 0, 0, 0);
            total += result;
        }
        return Long.toString(total);
    }

    public long memoizedRecurse(HashMap<Coord3, Long> memo, String map, int[] counts, int index, int curGroup, int countIndex) {
        ...
        if(map.charAt(index) == '#')
            return memoizedRecurse(memo, map, counts, index+1, curGroup+1, countIndex);
        if(map.charAt(index) == '.' || countIndex == counts.length) {
            ...
            if (countIndex < counts.length && curGroup == counts[countIndex]) {
                return memoizedRecurse(memo, map, counts, index + 1, 0, countIndex + 1);
                //keep going till end - hope we dont run into more #
            } else if (curGroup == 0) {
                return memoizedRecurse(memo, map, counts, index + 1, 0, countIndex);
                //incorrect!
            } else {
                return 0;
            }
        ...
        long springCount = memoizedRecurse(memo, map, counts, index+1, curGroup+1, countIndex);
        long nonCount = 0;
        //cases where dot is valid:
        if(curGroup == counts[countIndex])
            nonCount = memoizedRecurse(memo, map, counts, index+1, 0, countIndex+1);
        else if(curGroup == 0)
            nonCount = memoizedRecurse(memo, map, counts, index+1, 0, countIndex);
    ...
    public String part2() {
        ...
        for(String line: input.split("\n")) {
            ...
            //pad to avoid a silly ending issue - doesn't cause problems
            long result = memoizedRecurse(new HashMap<>(), fullMap,fullCount, 0, 0, 0);
            total += result;
        }
        return Long.toString(total);
    }
